...
using UnityEngine;
/*
public class SecuredCopy4 : MonoBehaviour
{
    [Header("Rope Settings")]
    public int maxPointsInSegment = 100;
    public float maxPossibleLenght = 50;
    ...
    public void GenerateRope(Vector3 startingPosition, Vector3 endingPosition, bool pinged, bool grabbed)
    {
        if(pinged)
        {
    ...
    void MoveCorners()
    {
        for(int i = 1; i<segments.Count; i++)
        {
            ...
            if(Physics.Raycast(relocatedStart, -startingEdge.normal1,out hit1, ropeRadious*2, ropeLayers) && Physics.Raycast(relocatedEnd, -startingEdge.normal2,out hit2, ropeRadious*2, ropeLayers))
            {
                if(hit1.normal != -hit2.normal)
                {
                    ...
                    middle += (startingEdge.normal1+startingEdge.normal2)/2 * ropeRadious;
            ...
            if(Angle < cornerAngleTrehsold || startingEdge.normal1 == -startingEdge.normal2)
            {
                beforeSegment.inBetween.AddRange(currrentSegment.inBetween);
                beforeSegment.endingEdge = currrentSegment.endingEdge;
                segments.RemoveAt(i);
    ...
    void VisualizeSegments(List<Segment> segments, Vector3 endingPoint)
    {
        ...
        allPoints.Reverse();  
        ShowSmoothLineRenderer(allPoints); 
    }
    void ShowSmoothLineRenderer(List<Vector3> points)
    {
    ...
    float GeneratePoint(Segment segment, float currentFound, bool lastOne, float theoricEndLength)
    {   
        ...
        if(segmentLenght+currentFound >= targetLength && lastOne)
        {
            segmentLenght = (targetLength-currentFound);
            fakeStartingPoint = endingPoint-segmentDirection*(segmentLenght+distanceBetweenPoints);
        }
        int pointAmount;   
        ...
        ApplyGravity(temporalPoints);
        FirstToLast(temporalPoints.Count-1,temporalPoints, extra);
        segment.middlePoints = LastToFirst(temporalPoints.Count-1,temporalPoints, extra);
        float realLength = 0;
    ...
    void ApplyGravity(List<Vector3> pointsTemp)
    {
        for(int i = 0; i< pointsTemp.Count-1; i++)
        {
            ...
            float gravityMult =1- (Mathf.Clamp(ropeTension,0.95f,1f)*20-19);
    ...
    void FirstToLast(int fixedPoint,List<Vector3> positionsTemp, float extra)
    {
            ...
            else
            {
                ...
                if(canStrech)
                {
                    if(between>tempDistance)
                        trans1 += (between-tempDistance)*(directionForward).normalized;  
                }
                else
                    trans1 += (between-tempDistance)*(directionForward).normalized;  
    ...
    List<MiddlePoint> LastToFirst(int fixedPoint, List<Vector3> positionsTemp, float extra)
    {        
        ...
        for(int i = 1; i<positionsTemp.Count; i++)
        {
            ...
            else
            {
                ...
                float tempDistance = distanceBetweenPoints;
                if(i == positionsTemp.Count-2 && extra > 0)
                    tempDistance *= extra;
                Vector3 directionForward = trans2-trans1;
                if(middlePoint != null)
                {
    ...
    Edge CollisionDetectionBetweenPoints(ref Vector3 forwardPoint, Vector3 backPoint)
    {
        ...
        if(TwoWayRaycast(backPoint, directionVector, forwardPoint, -directionVector, out hit1, out hit2, directionVector.magnitude))
        {
            ...
            {
                return new Edge(middle, hit1.normal, hit2.normal,null);       
            }          
        }
