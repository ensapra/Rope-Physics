...
using UnityEngine;
/*
public class SecureCopy5 : MonoBehaviour
{
    ...
    {   
        ...
        FirstToLast(temporalPoints.Count-1,temporalPoints, extra, tens);
        ApplyGravity(temporalPoints, segment.inBetween, tens);
    ...
    void ApplyGravity(List<Vector3> pointsTemp, List<Vector3> currentPoints, float tens)
    {
    ...
    void FirstToLast(int fixedPoint,List<Vector3> positionsTemp, float extra, float tension)
    {
        ...
        for(int i = 1; i<max; i++)
        {
            ...
            else
            {
                ...
                if(i == 1 && extra > 0)
                    tempDistance *= extra;
                Vector3 directionForward =(trans2-trans1)*(1-tension)+(positionsTemp[0]-positionsTemp[max-1])*tension;    
                if(canStrech)
                {
